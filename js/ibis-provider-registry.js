...
var providers=[
  ...
  },
  {
    ...
    apiStatus:'ACCOUNT_VERIFIED_SUPABASE_DEPLOYMENT_PENDING',affiliateStatus:'NOT_APPLICABLE',payAsYouGo:false,prepaidRequired:false,enabled:false,costToIbis:'ZERO_COST_TO_IBIS',
    ...
    weightsAvailable:'YES_APACHE_2_0_VERIFIED_ON_OFFICIAL_MODEL_CARD',sourceAvailable:'YES_HUGGING_FACE_BLACK_FOREST_LABS',selfHostable:false,deploymentMethod:'CLOUDFLARE_HOSTED_INFERENCE_REST_API',hardwareRequirements:'NOT_APPLICABLE_HOSTED_BY_CLOUDFLARE',verificationSource:'Direct authenticated test against the live Cloudflare account, 2026-08-21 (see IBIS-MAP.md Sec 0.17) -- confirmed against developers.cloudflare.com/workers-ai/models/flux-1-schnell/ for the model catalog entry itself',
    lifecycleState:'EXECUTABLE',
    note:'Phase 9 (2026-08-21): a founder-authorized Cloudflare account is now connected to this execution environment (verified via `wrangler whoami`). Model catalog re-checked live (`wrangler ai models list`) -- this exact model id is still current, not renamed. A REAL generation call was made directly against the live Cloudflare API using this environment\'s own authenticated credentials (never exposed to a browser): HTTP 200, real ~528KB JPEG image returned (base64 in the JSON envelope\'s result.image field -- confirmed by direct decode, not assumed -- output format is JPEG, not PNG as earlier notes speculated), and the image was viewed directly and confirmed coherent, non-corrupt and on-topic. This proves the PROVIDER genuinely works. It does NOT yet make this capability ELIGIBLE for real site traffic: FTN\'s architecture requires the API token to stay server-side (never exposed to client JavaScript), so real users are served through supabase/functions/ibis-image-cloudflare, which remains undeployed pending a separate, still-missing Supabase deployment credential (unrelated to Cloudflare). enabled stays false until that specific deployment happens -- EXECUTABLE (provider proven real) is honestly distinct from ELIGIBLE (live for real users), the same distinction this registry has maintained throughout.'
  },
  {
    ...
    lifecycleState:'EXECUTABLE',
    note:'Same real verification as cloudflare-workers-ai-image-flux, same date: HTTP 200, a real ~89KB image returned directly as raw binary (not JSON -- this model\'s REST response is a raw body with an "image/png" content-type header, but the actual decoded magic bytes are JPEG (0xFFD8) -- a real, confirmed content-type/actual-format mismatch on Cloudflare\'s own API, not an FTN bug; supabase/functions/ibis-image-cloudflare\'s existing raw-binary-response branch already handles this correctly since it trusts the bytes, not the label). Image viewed directly and confirmed coherent and on-topic. Same enabled:false reasoning as the flux entry: the provider is proven real, but the browser-safe production path (the Supabase function) remains undeployed pending a separate credential.'
  },
  {
