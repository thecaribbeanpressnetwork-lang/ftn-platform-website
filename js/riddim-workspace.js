...
(function (global) {
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      '.riddim-badge{display:inline-block;padding:4px 8px;border-radius:999px;background:#f1f1f1;font-size:10px;font-weight:900;letter-spacing:.08em;text-transform:uppercase}',
      '.riddim-card--dark .riddim-badge{background:#232933;color:#fff}',
  ...
  function trackIntake(content, api) {
    ...
    content.innerHTML = '<div class="workspace-card">' +
      ...
      '<p class="u-max-60ch">Attach an authorized track, read supported embedded metadata locally, complete or correct the record, then save one reusable FTN music record for release preparation and future FTN music tools.</p>' +
    ...
    form.addEventListener('submit', function (e) {
      ...
      record.source = {
        ...
        detectedMetadata: detectedMeta ? detectedMeta.detected : {},
        confirmationState: 'user-reviewed'
      };

      var html = '<div class="workspace-output riddim-record"><h3>' + escapeHtml(record.fields.trackTitle) + ' — FTN Music Record</h3>' +
        '<p><strong>Status:</strong> user-reviewed browser record. Unknown fields remain blank; FTN does not invent metadata.</p><table><tbody>';
      fields.forEach(function (f) {
        html += '<tr><th>' + escapeHtml(f.label) + '</th><td>' + escapeHtml(record.fields[f.key] || '—') + '</td></tr>';
      });
      html += '<tr><th>Attached source</th><td>' + escapeHtml(attachedFile ? attachedFile.name : '—') + '</td></tr></tbody></table>' +
        global.FTN.WorkspaceShell.exportRowHTML('riddim-save', 'Save this FTN music record') + '</div>';
      ...
      global.FTN.WorkspaceShell.wireExportButtons(output, {
        ...
        txtBody: function () {
          return record.fields.trackTitle + ' — FTN Music Record\n\n' + fields.map(function (f) {
            return f.label + ': ' + (record.fields[f.key] || '—');
          }).join('\n');
        },
        richBody: record
      });
      ...
      document.getElementById('riddim-save').addEventListener('click', function () {
        global.FTN.IntegrationAdapter.submit('riddim-music-record', record).then(function (res) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    global.FTN.WorkspaceShell.init({
      ...
      build: function (content, api) {
        content.innerHTML = '<section class="riddim-hub"><div class="riddim-head"><span class="workspace-eyebrow">FTN RIDDIM</span><h1>Powering Caribbean music.</h1><p>One music platform for track preparation, rights-aware metadata, release workflows and the next generation of Caribbean creation and performance tools.</p></div>' +
          '<div class="riddim-grid">' +
          '<article class="riddim-card"><span class="riddim-badge">Working beta</span><h2>Track Intake &amp; Release Prep</h2><p>Attach an authorized track, read supported embedded metadata, complete credits/rights information, review ISRC and publishing fields, then save/export one canonical FTN music record.</p><button class="btn btn-primary" id="riddim-track-choice" type="button">Open Track Intake</button></article>' +
          '<a class="riddim-card riddim-card--dark" href="/riddim/dj/"><span class="riddim-badge">Public beta / prototype</span><h2>FTN DJ + DJ Tube</h2><p>The DJ workspace: profiles, controller support and the existing DJ Tube prototype. Production audio, hardware and AI capabilities remain clearly separated from prototype features until verified.</p><span class="riddim-link">Open FTN DJ →</span></a>' +
          '<article class="riddim-card"><span class="riddim-badge">Concept / unbuilt</span><h2>FTN DAW</h2><p>The future production workstation inside FTN Riddim. This branch is intentionally not presented as operational until a real DAW implementation exists.</p><span class="riddim-link">Planned product</span></article>' +
