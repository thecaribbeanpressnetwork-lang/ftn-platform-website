...
(function (global) {
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      '.riddim-head p{color:var(--color-text-muted,#9aa3b2);font-size:16px;line-height:1.65}',
      '.riddim-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}',
      '.riddim-card{display:block;text-align:left;text-decoration:none;color:inherit;background:linear-gradient(145deg,rgba(255,255,255,.055),rgba(255,255,255,.025));border:1px solid rgba(255,255,255,.13);border-radius:18px;padding:24px;min-height:240px;box-shadow:0 16px 38px rgba(0,0,0,.16)}',
      '.riddim-card--dark{background:#0d1015;color:#fff;border-color:#2d333d}',
      '.riddim-card h2{font-size:23px;margin:12px 0 7px}.riddim-card p{line-height:1.55;color:#aeb5c0}',
      ...
      '.riddim-rights h3{margin-top:0}.riddim-rights p{color:#b3bac4;line-height:1.55}',
  ...
  function trackIntake(content, api) {
    ...
    content.innerHTML = '<div class="workspace-card">' +
      ...
      fields.map(function (f) { var wide = f.type === 'textarea' ? ' workspace-field--wide' : ''; return '<div class="workspace-field' + wide + '"><label for="rd-' + f.key + '">' + escapeHtml(f.label) + (f.required ? '' : ' (optional)') + '</label>' + fieldInputHTML(f) + '</div>'; }).join('') +
      '</div>' +
      '<section class="riddim-rights"><h3>Rights and identifiers</h3><p>Enter only identifiers, ownership, publisher/administrator details and splits you can verify. Leave unknown fields blank. Saving a metadata record does not create, transfer or administer music rights.</p></section>' +
  ...
  document.addEventListener('DOMContentLoaded',function(){injectStyles();global.FTN.WorkspaceShell.init({productId:'riddim',mountId:'workspace-root',accentSmallVar:'--color-riddim',build:function(content,api){content.innerHTML='<section class="riddim-hub"><div class="riddim-head"><span class="workspace-eyebrow">FTN RIDDIM</span><h1>Powering Caribbean music.</h1><p>One Caribbean music workspace for track preparation, rights-aware metadata, local audio shaping, versioning and DJ performance tools.</p></div><div class="riddim-grid"><article class="riddim-card"><h2>Track Intake &amp; Release Prep</h2><p>Attach an authorized track, inspect supported embedded metadata, complete credits and rights information, then save or export one canonical FTN music record.</p><button class="btn btn-primary" id="riddim-track-choice" type="button">Open Track Intake</button></article><a class="riddim-card" href="/riddim/daw/"><h2>FTN DAW</h2><p>Load authorized local audio, shape gain, tempo and adjustable EQ, compare the original, save named versions, then download the processed audio and its FTN settings recipe.</p><span class="riddim-link">Open FTN DAW →</span></a><a class="riddim-card riddim-card--dark" href="/riddim/dj/"><h2>FTN DJ Tube</h2><p>Open the two-deck Caribbean DJ workspace with local profiles, controller mapping, tempo, cue, crossfade and performance controls.</p><span class="riddim-link">Open FTN DJ Tube →</span></a></div></section><div id="riddim-track-mount" hidden></div>';document.getElementById('riddim-track-choice').addEventListener('click',function(){document.querySelector('.riddim-hub').hidden=true;var mount=document.getElementById('riddim-track-mount');mount.hidden=false;trackIntake(mount,api);});}});});
})(window);
