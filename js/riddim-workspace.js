...
(function (global) {
  ...
  document.addEventListener('DOMContentLoaded',function(){injectStyles();global.FTN.WorkspaceShell.init({productId:'riddim',mountId:'workspace-root',accentSmallVar:'--color-riddim',build:function(content,api){content.innerHTML='<section class="riddim-hub"><div class="riddim-head"><span class="workspace-eyebrow">FTN RIDDIM</span><h1>Powering Caribbean music.</h1><p>One Caribbean music workspace for track preparation, rights-aware metadata, production, versioning, creator identity and DJ performance tools.</p></div><div class="riddim-grid"><article class="riddim-card"><span class="riddim-card__icon riddim-icon--track" aria-hidden="true"></span><h2>Track Intake &amp; Release Prep</h2><p>Attach an authorized track, inspect supported embedded metadata, complete credits and rights information, then save or export one canonical FTN music record.</p><button class="btn btn-primary" id="riddim-track-choice" type="button">Open Track Intake</button></article><a class="riddim-card" href="/riddim/daw/"><span class="riddim-card__icon riddim-icon--daw" aria-hidden="true"></span><h2>FTN DAW</h2><p>Producer workspace for authorized local audio: gain, tempo, adjustable EQ, original comparison, named versions and downloadable output.</p><span class="riddim-link">Open FTN DAW →</span></a><a class="riddim-card riddim-card--dark" href="/riddim/dj/"><span class="riddim-card__icon riddim-icon--dj" aria-hidden="true"><i></i></span><h2>FTN DJ</h2><p>Two-deck Caribbean DJ Tube workspace with cue, crossfade, levels and optional browser MIDI controller mapping.</p><span class="riddim-link">Open FTN DJ →</span></a><a class="riddim-card riddim-card--epk" href="/radio/#ftn-epk"><span class="riddim-card__icon riddim-icon--epk" aria-hidden="true"></span><h2>FTN EPK</h2><p>Build reusable creator identity metadata, professional links, bio and press-photo references for FTN and authorized submissions.</p><span class="riddim-link">Build FTN EPK →</span></a></div></section><div id="riddim-track-mount" hidden></div>';document.getElementById('riddim-track-choice').addEventListener('click',function(){document.querySelector('.riddim-hub').hidden=true;var mount=document.getElementById('riddim-track-mount');mount.hidden=false;trackIntake(mount,api);});}});});
})(window);