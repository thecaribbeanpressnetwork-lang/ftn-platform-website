...
(function (global) {
  ...
  function renderRecent(host) {
    ...
    host.innerHTML = '<div class="dn-history">' + items.slice(-4).reverse().map(function (item) {
      ...
      return '<article class="dn-history__item"><strong>' + escapeHtml(p.venue || 'Venue') + '</strong>' +
        '<span>' + escapeHtml(p.venueType || '') + (p.country ? ' · ' + escapeHtml(p.country) : '') + '</span>' +
        '<small>' + escapeHtml((p.purposes || []).join(', ') || 'Deployment interest') + '</small></article>';
    }).join('') + '</div>';
  }
      ...
      build: function (content, api) {
        ...
        content.innerHTML =
          '<section class="dn-hero">' +
            '<div class="dn-hero__copy"><span class="dn-kicker">PHYSICAL DIGITAL INFRASTRUCTURE</span>' +
            '<h2>Turn screens into a trusted Caribbean network.</h2>' +
            '<p>Plan a venue deployment, define what the screens need to do, and create a structured brief FTN can use for real partnership conversations. This page does not claim a deployed network where one does not yet exist.</p></div>' +
            '<div class="dn-mnemonic" data-dn-mnemonic aria-hidden="true"><span class="dn-screen dn-screen--a"></span><span class="dn-screen dn-screen--b"></span><span class="dn-screen dn-screen--c"></span><span class="dn-screen dn-screen--d"></span><span class="dn-link dn-link--1"></span><span class="dn-link dn-link--2"></span><span class="dn-node"></span></div>' +
          '</section>' +
          ...
          '<section class="dn-planner"><div class="dn-planner__intro"><span class="dn-kicker">DEPLOYMENT DESK</span><h3>Build a venue deployment brief.</h3><p>Capture the minimum information needed to evaluate fit without locking FTN to a signage vendor, CMS or hardware stack.</p></div>' +
          '<form id="dn-form" novalidate>' +
            '<div class="dn-form-grid">' +
              '<div class="workspace-field"><label for="dn-venue">Venue / organization name</label><input type="text" id="dn-venue" name="venue" required></div>' +
              ...
              '<div class="workspace-field"><label for="dn-power">Power / mounting readiness</label><select id="dn-power" name="power"><option>Existing screens / power ready</option><option>Power available, screens needed</option><option>Site work likely required</option><option>Not yet assessed</option></select></div>' +
            '</div>' +
            ...
            '<label class="dn-consent"><input type="checkbox" name="followUp"> I want FTN to consider this brief for a future commercial conversation.</label>' +
            '<button type="submit" class="btn btn-primary">Save deployment brief</button>' +
          '</form><div id="dn-output"></div></section>' +
        ...
        form.addEventListener('submit', function (e) {
          ...
          var payload = {
            venue: form.venue.value.trim(), venueType: form.venueType.value,
            country: form.country.value.trim(), area: form.area.value.trim(),
            screenCount: form.screenCount.value || null, environment: form.environment.value,
            connectivity: form.connectivity.value, power: form.power.value,
            purposes: purposes, audience: form.audience.value.trim() || null,
            notes: form.notes.value.trim() || null, followUpRequested: !!form.followUp.checked,
            source: 'ftn-display-network-deployment-desk', capturedAt: new Date().toISOString()
          };
          var errors = [];
