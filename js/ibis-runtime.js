...
// One callable surface that joins personal context, routing, permissions and multi-agent execution.
...
(function(global){
  ...
  async function status(){var memories=await recall(null),permissions=FTN.PermissionLedger?await FTN.PermissionLedger.list():[],connections=FTN.AppRegistry?await FTN.AppRegistry.list():[];return{ready:!!(FTN.UniversalRouter&&FTN.MultiAgentOrchestrator&&FTN.PersonalContext&&FTN.PermissionLedger&&FTN.AppRegistry),memoryCount:memories.length,permissionCount:permissions.length,connections:connections.map(function(x){return{provider:x.provider,state:x.connection_state,scopes:x.scopes||[]};})};}
  FTN.IbisRuntime={ask:ask,remember:remember,recall:recall,permit:permit,permission:permission,connect:connect,status:status};
