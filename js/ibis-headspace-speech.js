...
(function (global) {
  ...
  async function play(){if(loading)return;if(audio&&audio.src){await audio.play();return;}var text=answerText();if(!text){status('There is no ibis answer to read yet.');return;}loading=true;controls.play.disabled=true;status('Preparing the approved founder voice…');try{var response=await fetch(ENDPOINT,{method:'POST',headers:{'content-type':'application/json',apikey:KEY},body:JSON.stringify({action:'speak',text:text})}),body=await response.json().catch(function(){return{};});if(!response.ok||!body.audio||body.voiceIdentity!=='IBIS_FOUNDER_VOICE'||body.founderListeningApproved!==true)throw new Error(body.error||'Founder voice unavailable.');audio=new Audio(decode(body.audio,body.mimeType||'audio/wav'));audio.playbackRate=rates[rateIndex];audio.onended=function(){status('Founder voice playback finished.');};audio.onerror=function(){status('The founder voice could not be played. The text answer remains available.');};await audio.play();status('Playing Ricardo’s approved founder voice.');}catch(error){status('Founder voice unavailable. No generic voice was substituted. '+(error&&error.message||String(error)));}finally{loading=false;controls.play.disabled=false;}}
