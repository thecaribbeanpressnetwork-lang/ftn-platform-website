...
// Critical invariant: LIVE data is never silently converted into a snapshot. A live presentation
// must retain its update/subscription path and display live state/source freshness visibly.
(function(global){
  'use strict';
  var MODES=['LIVE','SNAPSHOT','DERIVED','SCENARIO'];
  ...
  function normalize(spec){
    spec=spec||{};var mode=String(spec.mode||'SNAPSHOT').toUpperCase();if(!MODES.includes(mode))return fail('INVALID_MODE','Presentation mode must be LIVE, SNAPSHOT, DERIVED or SCENARIO.');
    if(mode==='LIVE'&&typeof spec.subscribe!=='function'&&typeof spec.refresh!=='function')return fail('LIVE_WITHOUT_UPDATE_PATH','LIVE presentation requires a subscribe() or refresh() update path; ibis will not relabel a static snapshot as live.');
  ...
  function render(container,spec){
    ...
    var stop=null;if(p.mode==='LIVE'){if(p.subscribe){stop=p.subscribe(function(update){if(update&&Array.isArray(update.values))p.values=update.values.slice();if(update&&Array.isArray(update.labels))p.labels=update.labels.slice();if(update&&update.sourceReferenceDate)p.sourceReferenceDate=update.sourceReferenceDate;if(update&&update.retrievedAt)p.retrievedAt=update.retrievedAt;draw();});}else if(p.refresh){var timer=setInterval(function(){Promise.resolve(p.refresh()).then(function(update){if(update&&Array.isArray(update.values))p.values=update.values.slice();if(update&&Array.isArray(update.labels))p.labels=update.labels.slice();draw();}).catch(function(){});},spec.refreshMs||60000);stop=function(){clearInterval(timer);};}}
