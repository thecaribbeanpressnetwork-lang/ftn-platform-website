...
(function () {
  ...
  // "not enough room," not shorter labels). The FULL product list still comes from the registry
  // itself, see populateEcosystemMenus() below.
  //
  // Phase 3 nav consolidation (2026-08-24 founder decision, BUILD NOW): PRIMARY_NAV below is
  // generated -- see data/nav-config.mjs (the ordered, curated list of what belongs in the
  // primary row) and scripts/sync-nav.mjs (which resolves it against js/product-registry-data.js
  // and writes the literal array here). Edit data/nav-config.mjs, not this array directly --
  // tests/nav-registry-audit.mjs fails CI if they drift apart. Kept as a plain synchronous JS
  // literal on purpose: the primary row must render on first paint with no registry fetch, so
  // only the *authoring* is registry-driven, never the runtime.
  //
  // This restores the 8-product, 11-item structure (FTN Platform, FTN Community Connect, FTN
  // Live, FTN Parliament, FTN TV, FTN Kaiso, FTN Riddim, FTN Invest-in, plus FTN Directory/About
  // FTN/Contact) as the founder's explicitly approved public structure, superseding the "Founder
  // Walkthrough Repair Pass" cut-to-5 note this comment used to carry. That prior pass cut the
  // list because 11 permanently-visible items wrapped the header actions cluster to a second row
  // at wide viewports -- this pass re-verified breakpoint/overflow behavior with the restored
  // list (see GOVERNANCE/FTN_Repair_Ledger_2026-08-24.md) rather than silently reintroducing the
  // regression it fixed.
  // FTN:NAV:START (generated by scripts/sync-nav.mjs -- do not hand-edit, edit data/nav-config.mjs) //
  var PRIMARY_NAV=[
    ...
    ['FTN Directory','/applications/','Browse FTN products, capabilities and data services'],
  ...
  function normalizeNavigation(){
    document.querySelectorAll('.site-nav').forEach(function(nav){
      ...
      // Restoring the founder-approved 11-item primary structure (Phase 3 nav consolidation)
      // reintroduced the exact overflow this header previously avoided by cutting the list to 5
      // items -- 11 real items plus the Ecosystem trigger don't fit at 1240-1600px viewports.
      // Rather than cutting the list again, the primary <ul> is its own horizontally-scrollable
      // flex item (see nav.css's .site-nav__list min-width:0/overflow-x rules); the Ecosystem
      // trigger is a SIBLING of the <ul>, not an <li> inside it, specifically so it stays outside
      // that scroll container -- otherwise its own overflow-x:auto ancestor would clip the mega
      // dropdown's height (overflow-x:auto forces overflow-y:auto too, per the CSS spec).
      nav.innerHTML='<ul class="site-nav__list">'+
        ...
        '</ul>'+
        '<div class="site-nav__item site-nav__item--ecosystem" data-nav-item><button type="button" class="site-nav__trigger" data-nav-trigger aria-haspopup="true" aria-expanded="false">FTN Ecosystem<svg class="chevron" viewBox="0 0 12 8" aria-hidden="true"><path d="M1 1l5 5 5-5" fill="none" stroke="currentColor" stroke-width="1.6"/></svg></button><div class="site-nav__dropdown site-nav__dropdown--mega" data-ecosystem-menu-panel><p class="ecosystem-menu__loading">Loading FTN Ecosystem…</p></div></div>';
    });
