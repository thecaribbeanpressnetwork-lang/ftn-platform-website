...
(function (global) {
  ...
  function initDrag(el){var handle=el.querySelector('.presentation-control__handle'),dragging=false,offsetX=0,offsetY=0;handle.addEventListener('pointerdown',function(e){dragging=true;var rect=el.getBoundingClientRect();offsetX=e.clientX-rect.left;offsetY=e.clientY-rect.top;try{handle.setPointerCapture(e.pointerId);}catch(err){}el.classList.add('presentation-control--dragging');});handle.addEventListener('pointermove',function(e){if(!dragging)return;var rect=el.getBoundingClientRect(),pos=clamp(e.clientX-offsetX,e.clientY-offsetY,rect.width,rect.height);placeAt(pos.x,pos.y);});function endDrag(){if(!dragging)return;dragging=false;el.classList.remove('presentation-control--dragging');var rect=el.getBoundingClientRect();savePosition({x:rect.left,y:rect.top});}handle.addEventListener('pointerup',endDrag);handle.addEventListener('pointercancel',endDrag);}
  function remove(){if(control&&control.parentNode)control.parentNode.removeChild(control);control=null;}
  function render(){if(control||dismissedThisView)return;control=document.createElement('div');control.className='presentation-control';control.setAttribute('role','region');control.setAttribute('aria-label','Presentation Mode control');control.innerHTML='<span class="presentation-control__handle" aria-hidden="true"><span class="presentation-control__dot"></span></span><span class="presentation-control__label">Presentation Mode</span><button type="button" class="presentation-control__exit">Exit to Live Mode</button><button type="button" class="presentation-control__dismiss" aria-label="Hide this control">&times;</button>';document.body.appendChild(control);var saved=loadPosition();if(saved){var rect=control.getBoundingClientRect(),pos=clamp(saved.x,saved.y,rect.width,rect.height);placeAt(pos.x,pos.y);}control.querySelector('.presentation-control__exit').addEventListener('click',function(){global.FTN.PlatformMode.set('live');global.location.reload();});control.querySelector('.presentation-control__dismiss').addEventListener('click',function(){dismissedThisView=true;remove();});initDrag(control);}
