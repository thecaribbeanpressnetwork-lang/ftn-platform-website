...
const sceneKey='ibisDeskPreviewSceneV1';
let topZ=10;
let history=[];
let future=[];
...
function saveScene(){try{localStorage.setItem(sceneKey,JSON.stringify(snapshot()));if(hint)hint.textContent='Saved this desk to this browser. Say “recall my desk” to bring it back.';}catch{if(hint)hint.textContent='This browser would not allow the prototype to save the desk.';}}
function recallScene(){try{const raw=localStorage.getItem(sceneKey);if(!raw){if(hint)hint.textContent='No saved desk exists in this browser yet.';return;}remember();apply(JSON.parse(raw));remember();if(hint)hint.textContent='Recalled your saved desk.';}catch{if(hint)hint.textContent='The saved desk could not be restored.';}}
...
document.getElementById('saveDeskTop')?.addEventListener('click',saveScene);
document.getElementById('recallScene')?.addEventListener('click',recallScene);
...
function interpret(text){const q=text.toLowerCase().trim();if(!q)return false;if(/^(go )?back$|undo|previous/.test(q)){undo();return true;}if(/forward|redo/.test(q)){redo();return true;}if(/save (this|my|the)? ?desk|remember this|save this view/.test(q)){saveScene();return true;}if(/recall (my|the)? ?desk|show my saved desk|bring back my desk|load my desk/.test(q)){recallScene();return true;}if(/put (that|it) back|bring (that|it) back|restore/.test(q)){restoreLast();return true;}if(/clear (the )?desk|hide everything|send everything away/.test(q)){hideCards(cards.map(card=>card.dataset.card),'Desk cleared.');return true;}const dictionary={graph:['graph','chart','demand'],opportunity:['opportunity','opportunities','funding','capital'],context:['context','signals','weather'],synapse:['connection','connections','synapse'],answer:['answer','response']};for(const [name,terms] of Object.entries(dictionary)){if(terms.some(term=>q.includes(term))){if(/hide|remove|send away|close/.test(q))hideCards([name],`Sent ${name} away.`);else showCards([name],`Recalled ${name}.`);return true;}}return false;}
