...
(function (global) {
  ...
  function renderHistory() {
    ...
    if (!records.length) { mount.innerHTML = '<p class="workspace-muted">No programming briefs saved on this device yet.</p>'; return; }
    mount.innerHTML = '<div class="radio-history-list">' + records.map(function (record) {
      ...
      return '<div class="radio-history-item"><span>' + escapeHtml(p.format || 'Segment') + '</span><strong>' + escapeHtml(p.title || 'Untitled') + '</strong><small>' + escapeHtml(when) + '</small></div>';
    }).join('') + '</div>';
  }

  document.addEventListener('DOMContentLoaded', function () {
    global.FTN.WorkspaceShell.init({
      productId: 'radio', mountId: 'workspace-root', accentSmallVar: '--color-radio',
      build: function (content, api) {
        var attachedFile = null;
        content.innerHTML = '<section class="radio-hero"><div><span class="radio-kicker">The Soundtrack of the Caribbean</span><h2>Build the programme before you go on air.</h2><p>Shape a Caribbean music, talk or culture programme, preview authorized local audio and save a structured production brief.</p></div><div class="radio-mnemonic" aria-hidden="true"><span class="radio-mnemonic__dial"></span><span class="radio-mnemonic__needle"></span><span class="radio-mnemonic__wave"></span></div></section>' +
          '<section class="radio-panel"><span class="radio-kicker">Programming Desk</span><h3>Create a show or segment brief</h3><form id="radio-form" class="radio-form" novalidate>' +
          ...
          '<div class="workspace-field"><label for="radio-notes">Programme concept</label><textarea id="radio-notes" name="notes" placeholder="In your own words: what happens in the programme, who it serves, and why it matters?" required></textarea></div>' +
          '<div class="workspace-field"><label for="radio-music">Music / talk direction <span class="workspace-field__hint">(optional)</span></label><textarea id="radio-music" name="music" placeholder="Genres, eras, artists, talk topics, recurring features or cultural focus"></textarea></div>' +
          ...
          '<div class="workspace-field"><label>Attach an audio recording <span class="workspace-field__hint">(optional local preview)</span></label><div id="radio-media-intake"></div><p class="workspace-field__hint">The preview stays on this device.</p></div>' +
          '<button type="submit" class="btn btn-primary">Save Programming Brief</button></form><div id="radio-output"></div></section>' +
          '<section class="radio-panel radio-history-panel"><span class="radio-kicker">Saved on this device</span><h3>Recent programming briefs</h3><div id="radio-history"></div></section>';
        ...
        form.addEventListener('submit', function (event) {
          ...
          var title = form.title.value.trim(), host = form.host.value.trim(), notes = form.notes.value.trim();
          var errors = [];
          ...
          var payload = { schemaVersion:2, title:title, host:host, format:form.format.value, duration:form.duration.value, territory:form.territory.value.trim(), mode:form.mode.value, notes:notes, music:form.music.value.trim(), frequency:form.frequency.value, rights:form.rights.value, attachedFile:attachedFile ? attachedFile.name : null, intentInterpreter:'ibis.ai' };
