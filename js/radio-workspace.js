...
var ef=document.getElementById('radio-link-form'),eo=document.getElementById('radio-link-output');ef.addEventListener('submit',function(e){e.preventDefault();
// FTN Consolidation (2026-09-18): the record shape and validation moved to js/ftn-epk-schema.js
// (a headless, no-DOM module) so ibis's EPK_GENERATION capability builds the exact same record --
// this page only collects the form fields and hands them to the shared schema.
var photos=[];['photo1','photo2','photo3'].forEach(function(k){var u=safeUrl(ef[k].value);if(u)photos.push(u);});
var EpkSchema=global.FTN.EpkSchema;
var formData={name:ef.name.value,email:ef.email.value,country:ef.country.value,roles:ef.roles.value,bio:ef.bio.value,photoReferences:photos,localPhotoMetadata:epkPhotoMetadata.slice()};
EpkSchema.LINK_KEYS.forEach(function(k){formData[k]=ef[k].value;});
var p=EpkSchema.buildRecord(formData);
var validationErrors=EpkSchema.validate(p);
if(validationErrors.length){eo.innerHTML=global.FTN.WorkspaceShell.renderErrorsHTML(['Creator name, professional email and bio are required.']);return;}
global.FTN.IntegrationAdapter.submit('radio',p).then(function(r){registerArtifact('ftn-epk-'+p.name.toLowerCase().replace(/[^a-z0-9]+/g,'-'),p.name+' — FTN EPK',p);eo.innerHTML='<div class="radio-link-preview"><span class="radio-kicker">FTN EPK Preview</span><h3>'+escapeHtml(p.name)+'</h3><p>'+escapeHtml(p.roles||'Caribbean creator')+' · '+escapeHtml(p.country||'Caribbean')+'</p><p>'+escapeHtml(p.bio)+'</p><p><strong>'+Object.keys(p.links).length+'</strong> professional links · <strong>'+(p.photoReferences.length+epkPhotoMetadata.length)+'</strong> press-photo references/selections.</p><p class="workspace-field__hint">Saved on this device. The structured EPK record stores metadata and references, not the local press-photo image files.</p></div>';api.notify(r.message,'success');pulse();renderHistory();});});renderHistory();if(location.hash==='#ftn-epk'){setTimeout(function(){var target=document.getElementById('ftn-epk');if(target){target.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'});target.focus({preventScroll:true});}},120);}}});});})(window);
