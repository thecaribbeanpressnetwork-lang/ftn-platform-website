...
function client(){if(!clientPromise)clientPromise=script().then(function(){return global.supabase.createClient(SUPABASE_URL,KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false,flowType:'pkce'}});});return clientPromise;}
// Supabase returns an authorization code after Google OAuth or an email magic link.
...
async function completeAuthRedirect(){var code='';try{code=new URL(global.location.href).searchParams.get('code')||'';}catch(e){}if(!code)return null;var c=await client(),result=await c.auth.exchangeCodeForSession(code);if(result.error)throw result.error;try{var u=new URL(global.location.href);u.searchParams.delete('code');global.history.replaceState({},document.title,u.pathname+(u.search||'')+(u.hash||''));}catch(e){}return result.data.session||null;}
// Provider failures return to FTN without an authorization code. Consume those query
// parameters once so a failed callback does not silently look like an ordinary guest page
// or keep replaying an error after refresh. Raw provider details stay out of the interface.
function consumeAuthCallbackError(){try{var u=new URL(global.location.href),error=u.searchParams.get('error')||'',code=u.searchParams.get('error_code')||'',description=u.searchParams.get('error_description')||'';if(!error&&!code&&!description)return null;['error','error_code','error_description','error_uri'].forEach(function(key){u.searchParams.delete(key);});global.history.replaceState({},document.title,u.pathname+(u.search||'')+(u.hash||''));var source=(error+' '+code+' '+description).toLowerCase(),kind=/access.denied|cancel/.test(source)?'cancelled':/invalid.client|exchange external code|provider/.test(source)?'provider_config':'callback';return{kind:kind};}catch(e){return null;}}
...
global.FTN=global.FTN||{};global.FTN.Auth={ready:client,completeAuthRedirect:completeAuthRedirect,completeEmailLink:completeAuthRedirect,consumeAuthCallbackError:consumeAuthCallbackError,getSession:getSession,getAccessToken:getAccessToken,getVerifiedUser:getVerifiedUser,signInWithEmail:signInWithEmail,signInWithGoogle:signInWithGoogle,verifyEmailOtp:verifyEmailOtp,signOut:signOut,invoke:invoke,ownerAccess:ownerAccess,enrollOwnerDevice:enrollOwnerDevice,claimOwnerDevice:claimOwnerDevice,ownerInvoke:ownerInvoke,clearOwnerDevice:clearOwnerDevice,onChange:onChange,safeReturn:safeReturn,rememberReturn:rememberReturn,returnPath:returnPath};
})(window);
