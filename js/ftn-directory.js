...
// Products/Capabilities/Data Services", not just list standalone Products and silently drop the
// 11 products that became absorbed capabilities. These two render alongside the existing
// per-topic Products groups on the full /applications/ page only (not the compact homepage
// ecosystem reveal, which stays a pure "what can I open right now" list by design).
function dataServiceCard(p){return'<article class="ftn-directory-card ftn-directory-card--data-service"><div class="ftn-directory-card__body"><div class="ftn-directory-card__title"><span class="ftn-directory-card__mark" aria-hidden="true"></span><h3>'+esc(p.name)+'</h3><span class="ftn-directory-card__status" data-status="DATA SERVICE">DATA SERVICE</span></div><p>'+esc(p.description)+'</p><small>Authoritative shared state other FTN products consume, not a conversational product of its own.</small></div><div class="ftn-directory-card__actions"><a href="'+esc(p.route)+'" aria-label="Open '+esc(p.name)+'">Open</a></div></article>';}
function capabilityCard(entry){var p=entry.product,parent=entry.absorbedIntoProduct,parentName=parent?parent.name:entry.product.absorbedInto,parentRoute=parent?parent.route:'/';return'<article class="ftn-directory-card ftn-directory-card--capability"><div class="ftn-directory-card__body"><div class="ftn-directory-card__title"><span class="ftn-directory-card__mark" aria-hidden="true"></span><h3>'+esc(p.name)+'</h3><span class="ftn-directory-card__status" data-status="ABSORBED">ABSORBED</span></div><p>'+esc(p.tagline||p.description)+'</p><small>Now part of '+esc(parentName)+'. Its legacy page stays live and explains the handoff.</small></div><div class="ftn-directory-card__actions"><a href="'+esc(parentRoute)+'" aria-label="Open '+esc(parentName)+'">Open '+esc(parentName)+'</a><a href="'+esc(p.route)+'" aria-label="Visit the legacy '+esc(p.name)+' page">Legacy page</a></div></article>';}
function dataServicesSection(products){if(!products.length)return'';return'<section class="ftn-directory-group ftn-directory-group--data-services" aria-labelledby="ftn-directory-data-services"><h3 id="ftn-directory-data-services">Data Services</h3><p>Verified shared state FTN products read from -- not a place to have a conversation, a place other products get one trustworthy number.</p><div class="ftn-directory-group__list">'+products.map(dataServiceCard).join('')+'</div></section>';}
function capabilitiesSection(entries){if(!entries.length)return'';return'<section class="ftn-directory-group ftn-directory-group--capabilities" aria-labelledby="ftn-directory-capabilities"><h3 id="ftn-directory-capabilities">Capabilities (absorbed into FTN products)</h3><p>These used to be standalone products. Nothing was deleted -- each one is now a real capability of the product named on its card, and its own page is still live for anyone who already has the link.</p><div class="ftn-directory-group__list">'+entries.map(capabilityCard).join('')+'</div></section>';}
function mount(host){
  ...
  // FTN Consolidation, closure wave (2026-09-18): only the full Directory page distinguishes
  // Products/Capabilities/Data Services -- the homepage's compact ecosystem reveal intentionally
  // stays a pure "what can I open right now" list.
  var extraSections=variant==='directory'&&Registry.dataServiceProducts&&Registry.absorbedCapabilities?dataServicesSection(Registry.dataServiceProducts())+capabilitiesSection(Registry.absorbedCapabilities()):'';
  host.innerHTML=intro+'<div class="ftn-directory__groups">'+groups.map(function(group){return groupMarkup(group,variant);}).join('')+extraSections+'</div>'+utility+'<p class="u-sr-only" id="ftn-directory-status" role="status" aria-live="polite"></p>';
