...
var GROUPS=[
  ...
  {title:'Community and infrastructure',ids:['events','love','display-network']},
  {title:'Shared access and future',ids:['account','health'],utility:true}
];
...
function mount(host){if(!global.FTN||!global.FTN.ProductRegistry)return;host.innerHTML='<div class="ftn-directory__intro"><span>FTN DIRECTORY</span><h2 id="ftn-directory-title">Explore the complete FTN ecosystem.</h2><p>Each product has one job, one truthful release state and a direct route. Mission Control remains private; FTN Account appears only as shared access.</p></div><div class="ftn-directory__groups">'+GROUPS.map(function(group){var products=group.ids.map(function(id){return global.FTN.ProductRegistry.get(id);}).filter(function(p){return p&&p.publicVisibility!==false&&p.status!=='PRIVATE';});return'<section class="ftn-directory-group'+(group.utility?' ftn-directory-group--utility':'')+'" aria-labelledby="ftn-directory-'+group.title.toLowerCase().replace(/[^a-z0-9]+/g,'-')+'"><h3 id="ftn-directory-'+group.title.toLowerCase().replace(/[^a-z0-9]+/g,'-')+'">'+esc(group.title)+'</h3><div class="ftn-directory-group__list">'+products.map(card).join('')+'</div></section>';}).join('')+'</div><p class="u-sr-only" id="ftn-directory-status" role="status" aria-live="polite"></p>';
