...
(function(global){
  ...
  function manager(field){
    ...
    // Investor-hardening fix (2026-09-19, confirmed via the IBIS Behavioral UX Gate test suite):
    // this counter used to grow without bound across a session's arrange()/drag/place() calls.
    // Once it climbed past 240 (the workspace menu's own fixed z-index -- see
    // .headspace-menu__panel in the base Headspace stylesheet), an ordinary, non-focused card
    // could render ABOVE an open menu dropdown and silently swallow clicks meant for its buttons
    // (reproduced directly: after ~250 arrange() calls, document.elementFromPoint() at the
    // "Freeform" button's own screen position resolved to <main class="headspace">, not the
    // button). Wrapping back to the low end once a safe ceiling is reached keeps every ordinary
    // card comfortably below the command hint (210), input orbit (220), window shelf (230) and
    // workspace menu (240) -- and well below focus/expand mode's own 500 -- while still giving
    // "the most recently arranged/dragged card is on top of its siblings" the same as before,
    // since a single arrange() pass never touches more than a handful of cards at once.
    function nextZ(){z=z>=199?30:z+1;return String(z);}
    ...
    function startDrag(e){
      ...
      node.style.zIndex=nextZ();node.classList.add('dragging');
