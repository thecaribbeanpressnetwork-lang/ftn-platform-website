...
- `ANTHROPIC_API_KEY` — required before the sitewide ibis widget (`ibis-assistant`) can answer; until set, the function returns a 503 and the widget shows its normal graceful-error state. `ANTHROPIC_MODEL` — optional override (code default is `claude-sonnet-5`; production sets this to `claude-sonnet-4-6`, a real, visible Anthropic model id — a 2026-09-18 note here calling it "never-valid" was itself wrong and is corrected in `docs/deferred-content.md`).
