...
// Supabase's platform JWT check accepts the project's public API key too. ibis
// is an account feature, so require a real authenticated-user access token.
...
Deno.serve(async (request) => {
  ...
  if (!hasAuthenticatedUser(request)) return reply({ error: "Sign in to use ibis AI." }, 401, origin);
