...
export const GATEWAY_VERSION = "ibis-gateway-2026-09-11.1";
...
function personLookupSubject(text: string): string | null {
  ...
  const match = raw.match(/^(?:tell me about|who is|who was|what do you know about|give me (?:information|info) (?:about|on))\s+(.+?)[?.!]*$/i);
  if (!match) return null;
  ...
  // Treat the request as a person lookup only when the user supplied a name-like proper noun.
  // This intentionally prefers a false-negative over blocking broad "tell me about X" topics.
  const nameLike = words.every((word) => /^[A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ'’-]*$/.test(word));
  return nameLike ? subject : null;
}
...
export function deterministicAnswer(text: string, products: IbisProduct[] = []) {
  ...
  if (person) return {
    answer: `I don’t have enough verified evidence about ${person} in this request to give you a reliable biography, profession or credits without guessing. I can only make those claims after a source-backed lookup.`,
    answerClass: "EVIDENCE_REQUIRED",
    evidenceState: "NO_VERIFIED_PERSON_EVIDENCE",
  };
...
/** Owned, zero-provider planning fallback. It is deliberately unavailable for general factual
 * questions: an unavailable model must never turn a fact request into a strategy-framework dump. */
...
export function gatewayHealth(providers: GatewayProvider[]) {
  const now = Date.now();
  return { gateway: "ready", version: GATEWAY_VERSION, deterministic: true, configuredProviders: providers.filter((p) => p.configured).length, availableProviders: providers.filter((p) => p.configured && circuitAllows(p.id, now)).length, providers: providers.map((p) => ({ id: p.id, label: p.label, configured: p.configured, available: p.configured && circuitAllows(p.id, now), model: p.model || null })), generatedAt: new Date(now).toISOString() };
}
...
export async function runGateway(input: { text: string; products?: IbisProduct[]; providers: GatewayProvider[]; requestId?: string }) {
  ...
  if (local) return { ...local, provider: "FTN ibis deterministic", model: "ibis-rules-v2", generatedAt: new Date().toISOString(), requestId, fallbackUsed: false, fallbackState: "NOT_NEEDED", confidence: "HIGH", uncertainty: local.evidenceState === "NO_VERIFIED_PERSON_EVIDENCE" ? "No sourced person evidence was supplied to this request." : null, gatewayVersion: GATEWAY_VERSION };
  let attempted = 0;
  ...
  for (const provider of input.providers) {
    ...
    try {
      ...
      if (leaksInternalFramework(result.answer)) throw new Error("POLICY_OUTPUT");
