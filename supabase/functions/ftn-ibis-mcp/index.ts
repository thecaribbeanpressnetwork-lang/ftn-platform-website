...
const PRICING_PAGE = "https://ftnplatform.org/ibis/pricing/";
...
const tools = [
  { name: "search", title: "Search FTN ibis", description: "Use this when the user wants source-backed Caribbean intelligence, entities, projects or opportunities. Returns only FTN-indexed records with provenance.", inputSchema: { type: "object", properties: { query: { type: "string", maxLength: 160 }, type: { type: "string", enum: TYPES }, limit: { type: "integer", minimum: 1, maximum: 20 } } }, annotations: { readOnlyHint: true, openWorldHint: false, destructiveHint: false, idempotentHint: true } },
  { name: "fetch", title: "Fetch an FTN ibis record", description: "Use this when the user wants the full source-backed record for an FTN ibis search result. Pass its record id or exact source URL.", inputSchema: { type: "object", properties: { id: { type: "string", maxLength: 160 }, sourceUrl: { type: "string", maxLength: 500 } }, anyOf: [{ required: ["id"] }, { required: ["sourceUrl"] }] }, annotations: { readOnlyHint: true, openWorldHint: false, destructiveHint: false, idempotentHint: true } },
  { name: "opportunity_scout", title: "Scout Caribbean opportunities", description: "Use this when the user wants grants, procurement, accelerators, awards or partnership opportunities relevant to the Caribbean. Scores are priority signals, not success probabilities.", inputSchema: { type: "object", properties: { query: { type: "string", maxLength: 160 }, territory: { type: "string", maxLength: 80 }, limit: { type: "integer", minimum: 1, maximum: 10 } } }, annotations: { readOnlyHint: true, openWorldHint: false, destructiveHint: false, idempotentHint: true } },
  { name: "route_intent", title: "Route an FTN intent", description: "Use this when the user expresses a Caribbean goal and needs the most relevant FTN product or ibis workflow. Routing is deterministic and transparent.", inputSchema: { type: "object", required: ["intent"], properties: { intent: { type: "string", minLength: 1, maxLength: 240 } } }, annotations: { readOnlyHint: true, openWorldHint: false, destructiveHint: false, idempotentHint: true } },
  { name: "get_entity_profile", title: "Get the FTN ibis profile", description: "Use this when the user asks what FTN ibis is, who owns it, what it connects to, or how to verify it.", inputSchema: { type: "object", properties: { entity: { type: "string", maxLength: 120 } } }, annotations: { readOnlyHint: true, openWorldHint: false, destructiveHint: false, idempotentHint: true } },
  { name: "get_service_tiers", title: "Compare FTN ibis Standard and Pro", description: "Use this when the user asks what FTN ibis costs or what the free and paid tiers include. This tool only describes plans; it cannot charge the user.", inputSchema: { type: "object", properties: {} }, annotations: { readOnlyHint: true, openWorldHint: false, destructiveHint: false, idempotentHint: true } },
];
...
function callTool(name: string, a: any = {}) {
  ...
  if (name === "get_service_tiers") return output("FTN ibis has a free Standard tier and an optional Pro pass.", { plans: [{ id: "ibis-standard", name: "FTN ibis Standard", price: "TT$0", features: ["Five public source-backed tools", "Caribbean opportunity search", "Provenance and original source links"] }, { id: "ibis-pro-30d", name: "FTN ibis Pro — 30 days", price: "TT$99", renewal: "One-time; no automatic renewal", features: ["Everything in Standard", "Private saved watchlists", "International capital brief matching"] }], pricingUrl: PRICING_PAGE, paymentNotice: "Checkout is WAM-hosted. This read-only tool cannot create a charge.", provenance: provenance({ lane: "service-tiers" }) });
