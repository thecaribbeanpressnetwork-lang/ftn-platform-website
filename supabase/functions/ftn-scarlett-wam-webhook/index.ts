...
// Modeled directly on supabase/functions/ftn-ibis-wam-webhook/index.ts -- same signature/timestamp
// verification, same idempotency ledger, same amount/currency mismatch check before ever writing
// an entitlement. See that file for the security rationale; this comment only calls out where
// Scarlett's version differs.
//
// Note on PAST_DUE: WAM as integrated here creates one-time payment intents for a fixed period
// (duration_days), not an auto-recurring subscription with card-on-file retries -- there is no WAM
// event type today (payment_intent.*) that represents a failed recurring charge. PAST_DUE exists in
// ftn_scarlett_entitlements' status column for forward-compatibility with a future recurring-
// billing capability, but nothing in this webhook ever sets it; it is reachable only by direct
// admin action. Renewal today means the user starts a new checkout before their period ends, which
// upsert's use of an existing "unexpired ends_at as the new base" already extends correctly.
...
Deno.serve(async(req:Request)=>{
  ...
  // Deliberately a distinct secret from WAM_WEBHOOK_SECRET (the ibis webhook's own): a Scarlett-
  // specific WAM webhook endpoint should be registered separately in the WAM dashboard so the two
  // products' payment events never share one verification secret.
  const secret=Deno.env.get("WAM_SCARLETT_WEBHOOK_SECRET")||"",url=Deno.env.get("SUPABASE_URL")||"",service=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")||"";
  ...
  const {error:insertError}=await admin.from("ftn_scarlett_payment_events").insert({event_id:eventId,event_type:type,provider_payment_id:String(data.paymentId||"").slice(0,255)||null,payload_sha256:hash});
  ...
  const fail=async(code:string,status=422)=>{await admin.from("ftn_scarlett_payment_events").update({processing_status:"REJECTED",processed_at:new Date().toISOString(),error_code:code}).eq("event_id",eventId);return json({error:code},status);};
  const paymentId=String(data.paymentId||""),reference=String(data.merchantReference||"");
  let orderQuery=admin.from("ftn_scarlett_payment_orders").select("id,user_id,plan_id,amount_minor,currency,status");
  if(/^[0-9a-f-]{36}$/i.test(paymentId))orderQuery=orderQuery.eq("provider_payment_id",paymentId);
  else if(/^FTNSCARLETT-[0-9]{13}-[0-9a-f]{8}$/i.test(reference))orderQuery=orderQuery.eq("order_reference",reference);
  ...
  if(order.status==="SUCCEEDED"&&next!=="SUCCEEDED"){await admin.from("ftn_scarlett_payment_events").update({processing_status:"PROCESSED",processed_at:new Date().toISOString(),error_code:"IGNORED_STATUS_REGRESSION"}).eq("event_id",eventId);return json({ok:true,ignored:true});}
  ...
  await admin.from("ftn_scarlett_payment_events").update({processing_status:"PROCESSED",processed_at:nowIso}).eq("event_id",eventId);
  return json({ok:true});
});
