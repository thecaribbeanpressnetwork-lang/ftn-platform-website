...
// candidate fields remain internal until first-party confirmation or another approved provenance gate.
...
Deno.serve(async(request)=>{
  ...
  const candidates=[];
  for(const element of elements){const tags=element.tags||{},name=(tags.name||"").trim();if(!name)continue;const sourceObject=`${element.type}/${element.id}`,ftnId=`FTN-${TERRITORY}-BIZ-OSM-${element.type[0].toUpperCase()}${element.id}`;candidates.push({ftn_id:ftnId,slug:`${slugify(name)}-${(await sha256(ftnId)).slice(0,8)}`,name,subcategory:tags.tourism||null,source_object:sourceObject,source_url:`https://www.openstreetmap.org/${sourceObject}`,source_hash:await sha256(`${SOURCE_KEY}:${sourceObject}`),fields:candidateFields(tags,element)});}
