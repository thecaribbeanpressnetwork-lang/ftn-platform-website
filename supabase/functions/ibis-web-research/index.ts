...
Deno.serve(async (req) => {
  ...
  if (payload.action === "health") return reply(origin, { capability: "WEB_RESEARCH", provider: "Google Search grounding via Gemini", model, configured: !!key, ready: !!key, inferenceAttempted: false, checkedAt: new Date().toISOString() });
  ...
  try {
    const upstream = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
      ...
      body: JSON.stringify({
        systemInstruction: { parts: [{ text: searchInstruction }] },
    ...
    if (!answer || !state.evidence.length) return reply(origin, { error: "Search completed but did not return enough grounded evidence to answer reliably.", answer: answer || null, sources: [], cebos: { requestClass: state.requestClass, caribbeanRelevant: state.caribbeanRelevant, sourcePlan: researchSourcePlan(state).slice(0, 6), evidence: evidenceSummary(state) } }, 424);
    return reply(origin, {
      answer,
      sources: state.evidence.map((e) => ({ title: e.claim, url: e.sourceUrl, publisher: e.publisher, sourceClass: e.sourceClass, evidenceStatus: e.status, discoveryUtility: e.discoveryUtility, decisionAuthority: e.decisionAuthority })),
      provider: "Google Search grounding via Gemini",
      model,
      retrievedAt: new Date().toISOString(),
      cebos: { version: state.version, requestClass: state.requestClass, caribbeanRelevant: state.caribbeanRelevant, sourcePlan: researchSourcePlan(state).slice(0, 6), evidence: evidenceSummary(state) },
    });
  } catch (error) {
    console.error("ibis-web-research failed", error);
    return reply(origin, { error: "Web research is temporarily unavailable." }, 502);
  }
});
