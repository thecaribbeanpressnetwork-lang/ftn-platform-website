...
async function bingSearch(query:string){
  try{
    const r=await fetch(`https://www.bing.com/search?q=${encodeURIComponent(query)}&format=rss`,{headers:{"user-agent":"Mozilla/5.0 (compatible; FTN-IBIS-Research/1.0)","accept":"application/rss+xml,application/xml,text/xml"},signal:AbortSignal.timeout(12000)});
    if(!r.ok)return[] as Array<{title:string;url:string;snippet:string}>;
    const xml=await r.text(),items=xml.match(/<item>[\s\S]*?<\/item>/gi)||[],out:Array<{title:string;url:string;snippet:string}>=[];
    for(const item of items.slice(0,10)){
      const title=decodeHtml(item.match(/<title>([\s\S]*?)<\/title>/i)?.[1]||"");
      const url=decodeHtml(item.match(/<link>([\s\S]*?)<\/link>/i)?.[1]||"");
      const snippet=decodeHtml(item.match(/<description>([\s\S]*?)<\/description>/i)?.[1]||"");
...
async function ddgSearch(query: string) {
  try{
    const r = await fetch(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`,{headers:{"user-agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/152 Safari/537.36","accept":"text/html"},signal:AbortSignal.timeout(12000)});
    if(!r.ok) return [] as Array<{title:string;url:string;snippet:string}>;
    const html = await r.text(),blocks = html.match(/<div[^>]+class="[^"]*result[^"]*results_links[^"]*"[\s\S]*?(?=<div[^>]+class="[^"]*result[^"]*results_links|<div id="links"|$)/gi) || [],out:Array<{title:string;url:string;snippet:string}>=[];
    for(const block of blocks.slice(0,10)){const a=block.match(/<a[^>]+class="[^"]*result__a[^"]*"[^>]+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/i);if(!a)continue;const sn=block.match(/class="[^"]*result__snippet[^"]*"[^>]*>([\s\S]*?)<\/a>/i)||block.match(/class="[^"]*result__snippet[^"]*"[^>]*>([\s\S]*?)<\/div>/i),target=resolveDdg(decodeHtml(a[1]));if(/^https?:\/\//i.test(target))out.push({title:decodeHtml(a[2]),url:target,snippet:decodeHtml(sn?.[1]||"")});}
    return out;
  }catch{return[] as Array<{title:string;url:string;snippet:string}>;}
}
...
Deno.serve(async (req) => {
  ...
  if(!answer||!state.evidence.length){
    ...
    if(enriched.length){answer=key?await synthesizeGemini(key,model,query,directive,enriched):"";if(!answer)answer=await synthesizeCloudflare(query,directive,enriched);provider=answer?"Independent web search + evidence synthesis":"Independent web search";}
  }
  ...
  const sources=state.evidence.map(e=>({title:e.claim,url:e.sourceUrl,publisher:e.publisher,sourceClass:e.sourceClass,evidenceStatus:e.status,discoveryUtility:e.discoveryUtility,decisionAuthority:e.decisionAuthority}));
  if(!answer||!state.evidence.length)return reply(origin,{error:"Web research completed without enough reliable evidence to answer.",providerStatus,providerCode,sources,cebos:compact},424);
  return reply(origin,{answer,sources,provider,model,retrievedAt:new Date().toISOString(),providerStatus,providerCode,cebos:compact});
});
