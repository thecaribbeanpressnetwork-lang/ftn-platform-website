...
| Program milestone name | "Executive Visual Polish & Caribbean Localization" — FTN wordmark brand correction (red T, per AEB-01), atmosphere refinement, and honest per-country messaging behind the v1.6 country-switcher scaffold. Ships together with the previously-unreleased 1.5.0 (Executive Design System) and 1.6.0 (Caribbean Executive Identity) passes — this is the first production update since v1.4.0 |
| Canonical release marker | git tag `v1.7.0` — authoritative; resolve it (`git rev-parse v1.7.0`) rather than trusting a hardcoded hash in this file, which necessarily can't name its own commit |
| Release date | 2026-07-13 |
| Branch | `main` |
| Remote / deployment | `origin` = `https://github.com/thecaribbeanpressnetwork-lang/ftn-platform-website.git`; Cloudflare Pages, custom domain `ftnplatform.org` — push/deploy verification results are recorded in this release's engineering report, not duplicated here |
