...
async function open(page,path,selector){
  // Headspace intentionally hydrates capabilities after first paint. Waiting for DOM/network idle
  // turns healthy background work into a false browser-test timeout, so gate on navigation commit
  // and the actual semantic control the user needs instead.
...
// FTN Quality/UX Pass (2026-09-18): the canonical entry prompt is now the mandated "What do you
// want to make happen?" phrase (matches js/ibis-ai-workspace.js's regular-chat welcome heading --
// see GOVERNANCE/FTN_Quality_Pass_2026-09-18.md), not the old "What do you need?" copy.
assert.match(await head.locator('[data-thought="answer"] h2').innerText(),/What do you want to make happen/i,'Headspace must open neutral.');
...
// FTN Quality/UX Pass (2026-09-18): Snap grid/Tile/Stack/Freeform moved off the always-visible
// toolbar into the workspace-menu <details> disclosure (mission requirement: "Remove unnecessary
// always-visible controls... window arrangement... move into three-dot menu or hamburger/menu
// panel") -- open the menu before an arrange button is clickable.
async function openWorkspaceMenu(){const menu=head.locator('#headspaceMenu');if(!(await menu.evaluate(el=>el.open))){await head.locator('#headspaceMenu > summary').click();}}
await openWorkspaceMenu();
const free=head.locator('[data-arrange="freeform"]');await free.waitFor({state:'visible'});await free.click();assert.equal(await head.locator('#field').getAttribute('data-layout'),'freeform');const card=head.locator('[data-thought="answer"]'),bar=card.locator('.thought-bar');const before=await card.boundingBox(),drag=await bar.boundingBox();assert(before&&drag);await head.mouse.move(drag.x+30,drag.y+15);await head.mouse.down();await head.mouse.move(drag.x+180,drag.y+120,{steps:8});await head.mouse.up();await head.waitForTimeout(150);const after=await card.boundingBox();assert(after&&Math.abs(after.x-before.x)>20);await openWorkspaceMenu();await head.locator('[data-arrange="grid"]').click();assert.equal(await head.locator('#field').getAttribute('data-layout'),'grid');await head.screenshot({path:'test-artifacts/ibis-behavioral-headspace.png',fullPage:true});

const main=await context.newPage();await isolate(main);const png='iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl2nXQAAAAASUVORK5CYII=';await main.route('**/functions/v1/ibis-image-cloudflare*',r=>r.fulfill({status:200,contentType:'application/json',body:JSON.stringify({image:png,mimeType:'image/png',extension:'png',provider:'Cloudflare Workers AI',model:'fixture-image'})}));await main.route('**/functions/v1/ftn-news-sources*',r=>r.fulfill({status:200,contentType:'application/json',body:JSON.stringify({localItems:[{title:'Trinidad fixture headline',publisher:'Fixture Trinidad Publisher',url:'https://example.test/trinidad-story',publishedAt:'2026-09-11T10:00:00Z',classification:'Publisher headline'}],items:[]})}));await open(main,'/ibis-ai/','#ibis-goal');
...
let bubble=await askMain('generate an image of a red scarlet ibis flying across the Caribbean');assert.match(await bubble.innerText(),/REAL IMAGE GENERATION/i);assert.equal(await bubble.locator('img').count(),1);bubble=await askMain('what is in the Trinidad news today?');const news=await bubble.innerText();assert.match(news,/Fixture Trinidad Publisher|Trinidad fixture headline/i);assert.doesNotMatch(news,/GitHub|Hacker News|IPTV/i);bubble=await askMain('tell me about Keyon Byron');const person=await bubble.innerText();assert.match(person,/not enough verified|could not verify|will not guess/i);assert.doesNotMatch(person,/musician|composer|producer/i);assert.equal(await main.locator('[data-ibis-headspace-entry] a[href="/ibis-headspace-preview/"]').count(),1);await main.screenshot({path:'test-artifacts/ibis-behavioral-workspace.png',fullPage:true});
