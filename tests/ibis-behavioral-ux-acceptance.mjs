...
async function mountHeadspace(page){
  ...
  for(const src of scripts){
    ...
    await page.addScriptTag({content:fs.readFileSync(path,'utf8')});
  }
...
async function open(page,path,selector){
  ...
  assert(response&&response.ok(),`Expected ${path} to return 2xx`);
...
await mountHeadspace(head);await head.waitForTimeout(500);
