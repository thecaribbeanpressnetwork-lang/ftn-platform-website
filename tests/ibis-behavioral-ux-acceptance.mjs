...
await head.locator('#headspaceMenu summary').click();const free=head.locator('[data-arrange="freeform"]');await free.waitFor({state:'visible'});await free.click();assert.equal(await head.locator('#field').getAttribute('data-layout'),'freeform');const card=head.locator('[data-thought="answer"]'),bar=card.locator('.thought-bar');const before=await card.boundingBox(),drag=await bar.boundingBox();assert(before&&drag);await head.mouse.move(drag.x+30,drag.y+15);await head.mouse.down();await head.mouse.move(drag.x+180,drag.y+120,{steps:8});await head.mouse.up();await head.waitForTimeout(150);const after=await card.boundingBox();assert(after&&Math.abs(after.x-before.x)>20);await head.locator('#headspaceMenu summary').click();await head.locator('[data-arrange="grid"]').click();assert.equal(await head.locator('#field').getAttribute('data-layout'),'grid');await head.screenshot({path:'test-artifacts/ibis-behavioral-headspace.png',fullPage:true});
