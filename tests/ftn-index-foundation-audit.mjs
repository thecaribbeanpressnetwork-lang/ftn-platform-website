...
const checks=[];function check(name,condition){checks.push({name,ok:Boolean(condition)});if(!condition)process.exitCode=1;}function read(path){return fs.readFileSync(new URL('../'+path,import.meta.url),'utf8');}
...
const operationsMigration=read('supabase/migrations/20260826230000_ftn_index_operations_and_outreach.sql');
const fn=read('supabase/functions/ftn-index/index.ts');const scout=read('supabase/functions/ftn-index-scout/index.ts');const ops=read('supabase/functions/ftn-index-ops/index.ts');const outreach=read('supabase/functions/ftn-index-outreach/index.ts');
const html=read('index/index.html');const page=read('js/ftn-index-page.js');const schema=read('js/ftn-index-schema.js');const opsHtml=read('god-mode/index/index.html');const opsPage=read('js/ftn-index-ops.js');const sitemap=read('sitemap.xml');
...
check('no-paid-provider-in-core-index',!/GEMINI_API_KEY|OPENAI_API_KEY|ANTHROPIC_API_KEY|RESEND_API_KEY/.test(fn+scout));
...
check('founder-can-pause-scout',/scout_enabled/.test(operationsMigration)&&/skipped:"founder_paused"/.test(scout));
check('outreach-disabled-by-default',/\('outreach_enabled','false'\)/.test(operationsMigration));
check('outreach-free-envelope',/3000/.test(outreach)&&/100/.test(outreach)&&/Cost Guard/.test(outreach)&&/resend-free/.test(operationsMigration));
check('outreach-pass-pilot-only',/selected_for_pilot/.test(outreach)&&/quality_status","pass/.test(outreach)&&/do_not_contact",false/.test(outreach));
check('outreach-token-created-at-send',/randomToken\(32\)/.test(outreach)&&/token_hash:tokenHash/.test(outreach)&&/revoked_at/.test(outreach));
check('recipient-optout-backend-only',/ftn_index_opt_out_invitation/.test(operationsMigration)&&/grant execute[\s\S]*to service_role/.test(operationsMigration)&&/claim-optout/.test(fn)&&!/Do not contact this business again/.test(page));
...
for(const item of checks)console.log(`${item.ok?'PASS':'FAIL'} ${item.name}`);if(process.exitCode)console.error(`FTN Index audit failed: ${checks.filter(x=>!x.ok).length} check(s).`);else console.log(`FTN Index audit passed: ${checks.length} checks.`);
