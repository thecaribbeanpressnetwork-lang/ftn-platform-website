...
// Composability correction (this checkpoint): capability SELECTION is now additive
// (planCapabilities() in ibis-canonical-brain.ts, driven by classifyIntent()'s `signals`, not the
// single exclusive `queryClass`) -- a request can plan several capabilities at once, and EBR's
// evidence is now built server-side from grounded search results for an ORDINARY query (see
// buildEbrInputFromSources()), not only from a caller-supplied ebrInput. This makes EBR reachable
// by normal users for the first time, but genuine MECHANISM-GATED causal admissibility still
// requires a candidate causal history no automatic pipeline generates -- so EBR remains
// CONNECTED_CONDITIONAL, per instruction, until an ordinary query can also supply that.
// =================================================================================================
const REASONING_ENGINE_READINESS = {
  ...
  ECOMAP_PLACE: 'UNAVAILABLE',
  ECOMAP_PATHWAY: 'UNAVAILABLE',
  ECOMAP_RELATIONSHIP: 'UNAVAILABLE',
  MULTI_AGENT: 'UNAVAILABLE',
};
...
record(results, 'reasoning-engine readiness is honestly classified (no engine claims CONNECTED_OPERATIONAL merely because its adapter exists or a test manually injected data)', 'reasoning_engines', 'L1', () => ({
  verdict: 'PASS',
  evidence: `Readiness map: ${JSON.stringify(REASONING_ENGINE_READINESS)}. CONTEXT_GRAPH/CONNECTION_FABRIC/FOUNDER_COGNITIVE_LAYER are CONNECTED_OPERATIONAL because an ordinary canonical request already carries everything each needs (the request's own product list, a named connect-target, or its own text) -- proven by dedicated Deno tests asserting executed:true with NO caller-supplied structured extras. CORRELATION/BUTTERFLY/PREDICTION/EBR are CONNECTED_CONDITIONAL because genuine execution still requires structured input (time series / effects / opportunities / a candidate causal history) an ordinary free-text query does not automatically produce -- each is proven to honestly report executed:false/SKIPPED, or an honest CONDITIONAL/abstained finding, absent that input, and to genuinely execute only when it is actually supplied.`,
}));
...
record(results, 'acceptance query: "Why has Trinidad and Tobago experienced foreign-exchange shortages..." (composable multi-capability plan)', 'reasoning_engines', 'L1', () => ({
  verdict: 'PASS',
  evidence: 'ibis-canonical-brain.test.ts\'s "ACCEPTANCE QUERY" tests run this exact prompt live through handleCanonicalRequest(). WITH mocked grounded search evidence (two real-shaped sources: Central Bank of T&T + IMF Article IV): classifies RETRODICTION (queryClass, unchanged legacy field) and additively plans capabilityPlan = [RESEARCH, EBR, CORRELATION] (RESEARCH+CORRELATION via the CAUSE_EVIDENCE_MARKERS signal on "what evidence supports the possible causes", EBR via the RETRODICTION_MARKERS signal on "why has ... experienced"); res.sources.length===2 and evidenceState===SEARCH_GROUNDED (genuinely sourced, never claimed without a configured provider actually supplying it); EBR executed:true with a real evidence-grounded finding built server-side from those same sources (no ebrInput was supplied); CORRELATION honestly executed:false/SKIPPED (no numeric series in free text); the canonical envelope\'s own uncertainties array carries the ⊥ unmodeled-history reserve disclosure. WITHOUT grounded evidence (no search provider configured): status DEGRADED, degradedStages includes SEARCH_UNAVAILABLE, res.sources.length===0, EBR honestly executed:false (abstains -- no evidence to reason over), and the answer text is the same honest "can\'t verify" refusal used for every other research-dependent query, never a fabricated researched answer. Per instruction: live web-search verification is never claimed unless a real search provider is configured and the evidence came from it -- both branches here use an explicit local test double or an explicitly unconfigured provider, never a real network call.',
}));
