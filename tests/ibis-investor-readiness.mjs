...
record(results, 'live provider canary (one minimal real search request)', 'real_search', 'BLOCKED_EXTERNAL', () => ({
  verdict: 'BLOCKED_EXTERNAL',
  evidence: 'No BRAVE_SEARCH_API_KEY and no live SEARXNG_BASE_URL exist anywhere in this environment (a PREVIEW SearXNG deployment is prepared in infra/searxng-preview/ but not yet deployed -- see the SearXNG preview record below). A real, live HTTP call to the deployed ibis-provider-health-preview Edge Function this checkpoint confirmed ANTHROPIC_API_KEY is configured server-side but Anthropic\'s own API rejects it (401) -- see the Claude Web Search record below. Google Search grounding remains confirmed NOT implemented anywhere in this codebase.',
  blocker: 'Three independent unblock paths, none requiring this assistant to hold a credential: (a) deploy the prepared SearXNG preview (infra/searxng-preview/README.md -- needs a founder Render or Hugging Face account) and set SEARXNG_BASE_URL; (b) fix the ANTHROPIC_API_KEY secret at https://console.anthropic.com/settings/keys (a console account, distinct from a claude.ai chat subscription) -- the Claude Web Search adapter is already implemented and will work the moment this authenticates; (c) create a Brave Search API account (https://brave.com/search/api/) and set BRAVE_SEARCH_API_KEY. See docs/ibis/acceptance-baseline.md\'s "Live Search Infrastructure" sections for full detail on each.',
}));
record(results, 'SearXNG PREVIEW deployment prepared (Render/Hugging Face) -- reuses the existing searxngSearch() adapter unmodified', 'real_search', 'L1', () => ({
  verdict: 'PASS',
  evidence: 'infra/searxng-preview/ (Dockerfile + settings.yml + README.md) is a ready-to-deploy PREVIEW configuration for the official searxng/searxng Docker image: JSON output enabled (required by the unmodified, already-existing searxngSearch() in ibis-search-adapter.ts), the built-in request limiter on, debug off, never listed as a public instance. Deployment itself requires a founder account on Render or Hugging Face (exact links/steps in that README) -- account creation on a third-party site is not performed by this assistant. Not built or run locally (no Docker in this sandbox) -- the README explicitly instructs verifying the JSON endpoint directly after deploy, before treating it as working. Documents cold-start, no-persistent-disk, and outbound-bandwidth limitations for both hosting options.',
}));
record(results, 'Claude Web Search adapter (server-only, Anthropic web_search_20250305 tool) -- implemented and unit-tested; ANTHROPIC_API_KEY CONFIGURED_BUT_FAILING, live-verified', 'real_search', 'L1', () => ({
  verdict: 'PASS',
  evidence: 'New supabase/functions/_shared/ibis-claude-search-adapter.ts requests Anthropic\'s real web-search tool (distinct from the existing plain-completion anthropic() provider, which never requests grounding) and normalizes real web_search_tool_result blocks into the same SourceRecord/SearchResult contract every other provider produces, reported as provider:"claude-web-search" (never "google" or "searxng"), so it feeds EBR/EcoMap/canonical reasoning identically. Capped at max_uses:1 by default; its own separate, more conservative daily/monthly budget than Brave\'s (real money per search, unlike SearXNG/Brave\'s free tier); wired into search()\'s cascade between SearXNG and Brave. 8 dedicated adapter tests plus 2 cascade tests prove: honest skip with zero network calls when no key; correct normalization with no fabricated publishedAt from Anthropic\'s relative page_age string; URL dedup across result blocks; no crash on an HTTP error or a web_search_tool_result_error; an honest SEARCH_UNAVAILABLE when Claude chooses not to search; max_uses:1 sent by default, raised only on explicit opt-in; correct cascade ordering and independent per-provider budget fallthrough. LIVE-VERIFIED THIS CHECKPOINT (not mocked): a real HTTP call to the deployed ibis-provider-health-preview function (using this repo\'s own public Supabase publishable key, already shipped in dozens of client-side files) returned {"provider":"Anthropic","state":"UNHEALTHY","httpStatus":401} -- that function returns NOT_CONFIGURED when a key is absent (confirmed by reading its source), so UNHEALTHY+401 proves a key IS set server-side but Anthropic\'s real API currently rejects it. Per instruction, not confused with a claude.ai chat subscription (no API entitlement) -- exact founder action: generate a fresh key at https://console.anthropic.com/settings/keys and update the secret. The required real current-Trinidad-and-Tobago-query end-to-end proof cannot run until that is fixed; not claimed otherwise.',
}));
