...
async function open(page,path,selector){const response=await page.goto(base+path,{waitUntil:'commit',timeout:15000});assert(response&&response.ok(),`Expected ${path} to return 2xx`);await page.locator(selector).waitFor({state:'attached',timeout:15000});}

const landing=await context.newPage();await isolate(landing);await open(landing,'/ibis-preview/index.html','#askInput');
...
await landing.locator('#askInput').fill('What is the latest USD selling rate?');await landing.locator('#askForm button[type="submit"]').click();await landing.waitForURL(/\/ibis-headspace-preview\/\?q=/,{waitUntil:'commit',timeout:15000});await landing.close();

const page=await context.newPage();await isolate(page);
...
await open(page,'/ibis-headspace-preview/index.html','#headspaceQuery');await page.waitForTimeout(500);
