...
assert.deepEqual(Array.from(products.find(p=>p.id==='ibis-ai').sitemapRoutes),['/ibis/','/ibis/ecosystem-map/','/ibis/international-capital/','/ibis/pricing/'],'FTN ibis discovery routes must remain registry-owned');
...
assert.match(fs.readFileSync('service-worker.js','utf8'),/VERSION='ftn-public-v2\.4\.4'/,'Service-worker cache namespace was not advanced for changed assets');
