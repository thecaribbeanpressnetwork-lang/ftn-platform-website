...
await scenario('kaiso-newsroom', async page=>{
  ...
  assert(!/Invalid Date/i.test(await page.locator('#kaiso-video-feed').innerText()),'Kaiso rendered an invalid provider date');assert.match(await page.locator('#kaiso-local-feed').innerText(),/Guardian.*Attributed publisher headline/is);assert.equal(await page.locator('a[href*="trinidadexpress.com"]').count(),1,'Express direct source missing');assert.equal(await page.locator('a[href*="youtube.com/c/IanAlleyne"]').count(),0,'unapproved local source lane rendered');
