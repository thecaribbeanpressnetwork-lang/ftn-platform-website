...
await scenario('daw-live-audio', async page=>{
  ...
  // A fixed 150ms wait here was flaky under load (the live change status text is real and correct,
  // just not always painted within a hardcoded delay) -- poll for it instead, like the trackName
  // wait above.
  await page.waitForFunction(()=>/Live change applied: gain/i.test(document.querySelector('#status')?.textContent||''),{timeout:4000});
...
await scenario('applications-progressive-directory',async page=>{await open(page,'/applications/');assert(await page.locator('.ftn-directory-card').count()>=20);assert.equal(await page.locator('.ftn-directory-card a[href="/love/"],.ftn-directory-card a[href="/health/"]').count(),0);
  // FTN Consolidation (2026-09-18): the new Capabilities section (Directory must "clearly
  // distinguish Products/Capabilities/Data Services") legitimately adds a second /govern/ link --
  // the absorbed FTN Parliament capability card correctly points to its new home, FTN Govern, in
  // addition to FTN Govern's own standalone product card. The original intent (no duplicate
  // identity for the SAME standalone product) is preserved by scoping to non-capability cards.
  assert.equal(await page.locator('.ftn-directory-card:not(.ftn-directory-card--capability):not(.ftn-directory-card--data-service) a[href="/govern/"]').count(),1);
  assert.equal(await page.locator('.ftn-directory-card--capability a[href="/govern/"]').count(),1,'FTN Parliament capability card should hand off to FTN Govern');
  assert.equal(await page.locator('main').getByText('Mission Control',{exact:true}).count(),0);assert.doesNotMatch(await page.locator('main').innerText(),/\bBETA\b/);});
