...
await scenario('home-desktop', async page=>{await open(page,'/');await page.waitForSelector('#find-your-path',{timeout:10000});assert.match(await page.locator('.eco-hero h1').innerText(),/Uniting the Caribbean/i);assert.match(await page.locator('#ibis-lead-title').innerText(),/AI that understands the Caribbean/i);assert.match(await page.locator('.eco-ibis-lead').innerText(),/image & photo projects/i);assert.match(await page.locator('.eco-ibis-lead').innerText(),/video projects/i);assert(await page.locator('a[href="/ibis-ai/"]').count()>0);assert(await page.locator('a[href="/riddim/"]').count()>0);assert(await page.locator('a[href="/riddim/fire/"]').count()>0);assert(await page.locator('a[href="/riddim/daw/"]').count()>0);assert(await page.locator('a[href="/riddim/dj/"]').count()>0);assert.equal(await page.locator('.eco-node').count(),21,'home must bill every public FTN node');assert.equal(await page.locator('.eco-node').first().locator('strong').innerText(),'ibis.ai','ibis must lead the public node directory');assert(await page.locator('.eco-live-rail').count()===1,'home live tools rail missing');assert.equal(await page.locator('a[href="/love/"]').count(),0,'PRIVATE Love must not be advertised publicly');assert.equal(await page.locator('.country-switcher-dialog.is-open').count(),0,'country selection must not be forced');});
