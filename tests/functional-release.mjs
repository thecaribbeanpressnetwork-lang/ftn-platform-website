...
await scenario('home-desktop', async page=>{await open(page,'/');await page.waitForSelector('#find-your-path',{timeout:10000});assert(await page.locator('a[href="/ibis-ai/"]').count()>0);assert(await page.locator('a[href="/govern/"]').count()>0);assert(await page.locator('a[href="/riddim/daw/"]').count()>0);assert(await page.locator('a[href="/riddim/dj/"]').count()>0);assert.equal(await page.locator('.eco-panel').count(),0,'homepage duplicated the artwork product wall');assert(await page.locator('.eco-live-rail').count()===1,'home live tools rail missing');assert.equal(await page.locator('.country-switcher-dialog.is-open').count(),0,'country selection must not be forced');const expected=['FTN Community Connect','FTN Face The Nation','FTN ibis.ai','FTN Riddim','FTN Kaiso','FTN Screen','FTN Opportunities','FTN Fire','FTN DJ Tube','FTN DAW'];assert.deepEqual(await page.locator('.eco-door>span').allTextContents(),expected,'homepage product hotspots must use full FTN brand names');assert.equal(await page.locator('.eco-door[aria-label^="Enter"]').count(),0,'homepage retained the old Enter metaphor');assert.equal(await page.locator('.eco-civic-link[href="/observatory/"] strong').innerText(),'FTN Live');assert.equal(await page.locator('.eco-civic-link[href="/parliament/"] strong').innerText(),'FTN Parliament');assert.equal(await page.locator('.eco-live-rail a[href="/parliament/"] strong').innerText(),'FTN Parliament');assert(await page.locator('.ftn-directory-card').count()>=22,'complete FTN Directory missing below homepage artwork');const text=await page.locator('body').innerText();assert(!/choose a room|art is the door/i.test(text),'homepage still uses the room/door metaphor');assert.equal(await page.getByRole('button',{name:'Support Us'}).count(),0,'outdated Support Us control remains');assert(await page.getByRole('link',{name:'FTN InvestIn',exact:true}).count()>0,'FTN InvestIn header path missing');});
await scenario('home-release-assets-versioned', async page=>{await open(page,'/');assert.match(await page.locator('link[href*="ecosystem-homepage-refinement.css"]').getAttribute('href'),/\?v=20260812\.1$/,'homepage refinement CSS is not cache-versioned');assert.match(await page.locator('script[src*="home-live-rail.js"]').getAttribute('src'),/\?v=20260812\.1$/,'homepage live rail is not cache-versioned');assert.match(await page.locator('script[data-ftn-platform-foundation]').getAttribute('src'),/\?v=20260812\.1$/,'homepage foundation is not cache-versioned');});
...
await scenario('applications-progressive-directory',async page=>{await open(page,'/applications/');assert(await page.locator('.ftn-directory-card').count()>=22);assert.equal(await page.locator('.ftn-directory-card a[href="/love/"]').count(),1);assert.equal(await page.locator('.ftn-directory-card a[href="/govern/"]').count(),1);assert.equal(await page.locator('main').getByText('Mission Control',{exact:true}).count(),0);assert.doesNotMatch(await page.locator('main').innerText(),/\bBETA\b/);});
...
await scenario('pwa-private-cache-exclusion', async page=>{await open(page,'/');var manifest=await page.evaluate(()=>fetch('/manifest.webmanifest').then(r=>r.json()));assert(!JSON.stringify(manifest).includes('/god-mode'),'public manifest advertises God Mode');assert(!JSON.stringify(manifest).includes('/love/'),'public manifest advertises private Love');var sw=await page.evaluate(()=>fetch('/service-worker.js').then(r=>r.text()));for(const route of ['god-mode','mission-control','account','love','ibis-ai'])assert(sw.includes(route),`service worker private-route exclusion missing: ${route}`);});
